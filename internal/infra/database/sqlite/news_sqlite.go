...
func (repo *NewsSQLiteRepository) Create(news entity.News) (entity.News, error) {    
    ...
    insertQuery := "INSERT INTO news (id, title, text, link, image) VALUES (?, ?, ?, ?, ?)"
    _, err := db.Exec(insertQuery, news.ID, news.Title, news.Text, news.Link, news.Image)
...
func (repo *NewsSQLiteRepository) Update(news entity.News) (entity.News, error)  {    
    ...
    if news.Title != "" || news.Text != "" || news.Link != "" || news.Image != "" {
		_, err := db.Exec(query)
		if err != nil {
			fmt.Println(err)
			return entity.News{}, err
...
func (repo *NewsSQLiteRepository) GetById(id string) (entity.News, error) {
    ...
    newsQuery := "SELECT * FROM new WHERE id = ?"
    row := db.QueryRow(newsQuery, id)    

    // Variáveis para armazenar os dados do usuário
    var title, text, link, image string
    var created_at, updated_at time.Time

    // Recuperando os valores do banco de dados
    err = row.Scan(&id, &title, &text, &link, &image, &created_at,  &updated_at)
    if err != nil {        
...
func (repo *NewsSQLiteRepository) FindAll(page, limit int) ([]entity.News, error) {
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&news.ID, &news.Title, &news.Text, &news.Link, &news.Image, &news.CreatedAt, &news.UpdatedAt)
        if err != nil {            
            return nil, err
        }
